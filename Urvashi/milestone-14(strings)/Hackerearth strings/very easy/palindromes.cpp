...
int main()
{
	...
	// if all char of string are equal then there is no way to convert into a substring which is not palindrome.
	if (x == 0)
	{
		cout << "0" << endl;
	}
	else if (b != s) //if string is not palindrome .
	{
		cout << strlen(s) << endl; // its only the mxm length substring.
					 ...
					 // after decreasing its length by one we get a substring of max length which is not a palindrome.
