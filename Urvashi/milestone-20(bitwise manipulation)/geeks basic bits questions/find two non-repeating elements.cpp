...
void non_repeating_2(int *a,int n,int *x , int *y)
{
	...
	*x=0,*y=0;
	...
	//now divide elements in two subsetsby comparing rightmost bit of xor with bit at same position in each element..
	for(int i=0;i<n;i++)
	{
		if(a[i]&set_bit_no)
...
int main()
{
	...
	cout<<"two non-repeting numbers are= "<<*x<<" "<<*y<<endl;
}
