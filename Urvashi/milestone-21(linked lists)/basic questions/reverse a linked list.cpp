...
struct node* Reverse(node* head)//now this time i have not declared the variable head globally so i have to pass it in every fxn 
{
	node *current ,*prev,*next ;// taking three pointers , current to store current position's address, next for next , and prev for previous.
